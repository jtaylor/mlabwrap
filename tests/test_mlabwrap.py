...
##         super(TestCase, self).assertEquals(res, True)
    ...
    def testBasic(self):
        ...
        for isComplex in [False, True]:
            ...
            for i in range(30):
                ...
                if isComplex:
                    a = a + 1j * numpy.random.random(dims)
                a1 = a.copy()
                print a.shape
                mlab._set('a', a)
                #### test simple get ####
    ...
    def testOrder(self):
        ...
        # JT - This used to be called without the type=float, which resulted in mlab.conj(fa) being integer
        # valued and mlabraw not knowing how to convert this back to a numpy array.
        # It was thus left as a Proxy object which caused quite a problem for the assertEqual below it.
        # I am not sure if this was the result, but adding the type=float allows the test to pass and
        # it seems to me that this might still be testing the ordering?
        import numpy
        fa = numpy.array([[1, 2, 3.], [4, 5, 6]], order='F', dtype=float)
        self.assertEqual(mlab.conj(fa), fa)

        # JT - Still need to grok this next line.
